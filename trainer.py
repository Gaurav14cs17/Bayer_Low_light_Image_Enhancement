...
class Trainer(object):
    ...
    def validate(self):
        ...
        for batch_idx, (raws, imgs, targets, img_files, img_exposures, lbl_exposures, ratios) in tqdm.tqdm(
                ...
                desc=f"{'Valid' if self.cmd == 'train' else 'Test'} iteration={self.iteration} epoch={self.epoch}",
                ncols=80, leave=False):

            gc.collect()
            if self.cuda:
                raws = raws.cuda(non_blocking=True)
            ...
            for output_img, img, target_img, img_file, *_ in zip(outputs, imgs, targets_cpu, img_files, img_exposures):
            ...
            if batch_idx % self.print_freq == 0:
                log_str = f"{'Valid' if self.cmd == 'train' else 'Test'}: [{batch_idx}/{len(self.val_loader)}/{losses.count}] " \
                          f"epoch: {self.epoch} iter: {self.iteration} " \
                          f"Time: {batch_time.val:.3f} ({batch_time.avg:.3f}) " \
                          f"Loss: {losses.val:.4f} ({losses.avg:.4f}) " \
                          f"PSNR: {psnr_meter.val:.2f} ({psnr_meter.avg:.2f}) " \
    ...
    def train_epoch(self):
        ...
        for batch_idx, (raws, imgs, targets, img_files, img_exposures, lbl_exposures, ratios) in tqdm.tqdm(
                ...
                desc=f'Train epoch={self.epoch} iter={self.iteration}', ncols=80, leave=False):
            ...
            targets_cpu = targets.cpu()
            for output_img, img, target_img, img_file, *_ in zip(outputs_cpu, imgs, targets_cpu, img_files, img_exposures):
                output_arr = (output_img.numpy().transpose(1, 2, 0) * 255).astype(np.uint8)
                target_arr = (target_img.numpy().transpose(1, 2, 0) * 255).astype(np.uint8)
                psnr_meter.update(utils.get_psnr(output_arr, target_arr), 1)

                if self.result_dir:
                    os.makedirs(os.path.join(self.result_dir, f"{self.epoch:04d}"), exist_ok=True)
                    fname = os.path.join(self.result_dir, f"{self.epoch:04d}/{batch_idx:04d}_{os.path.basename(img_file)[:-4]}.jpg")
                    temp = np.concatenate((target_arr, output_arr), axis=1)
            ...
            if self.iteration % self.print_freq == 0:
                log_str = f"Train: [{batch_idx}/{len(self.train_loader)}/{losses.count}] epoch: {self.epoch} iter: {self.iteration} " \
                          f"Time: {batch_time.val:.3f} ({batch_time.avg:.3f}) Data: {data_time.val:.3f} ({data_time.avg:.3f}) " \
                          f"Loss: {losses.val:.4f} ({losses.avg:.4f}) PSNR: {psnr_meter.val:.1f} ({psnr_meter.avg:.1f}) " \
...
class Validator(Trainer):
    def __init__(self, cmd, cuda, model, criterion, val_loader, log_file, result_dir=None, use_camera_wb=False, print_freq=1):
        super().__init__(cmd, cuda=cuda, model=model, criterion=criterion,
