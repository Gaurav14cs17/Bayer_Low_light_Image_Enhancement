...
class LargeUNet(nn.Module):
    def __init__(self, in_channels=4, out_channels=3, init_features=64):
        ...
        # Attention gates
        self.attention3 = AttentionGate(features *8, features*8)
        self.attention2 = AttentionGate(features *4, features*4)
        self.attention1 = AttentionGate(features*2, features *2)
    ...
    def forward(self, x):
        ...
        dec4 = torch.cat((self.attention3(enc4, dec4), dec4), dim=1)
        dec4 = self.decoder4(dec4)

        dec3 = self.upconv3(dec4)
        dec3 = torch.cat((self.attention2(enc3, dec3), dec3), dim=1)
        dec3 = self.decoder3(dec3)

        dec2 = self.upconv2(dec3)
        dec2 = torch.cat((self.attention1(enc2, dec2), dec2), dim=1)
        dec2 = self.decoder2(dec2)

        dec1 = self.upconv1(dec2)
        dec1 = torch.concat([enc1, dec1], dim=1)
