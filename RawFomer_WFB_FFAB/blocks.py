...
class FEB(nn.Module):
    ...
    def forward(self, x):
        _, _, H, W = x.shape
        x = torch.clamp(x, -10, 10)  # Input stabilization
        x_freq = torch.fft.rfft2(self.fpre(x), norm='ortho')
        mag = torch.abs(x_freq) + 1e-6  # Prevent zero
        pha = torch.angle(x_freq)
        mag = torch.clamp(self.process1(mag), 0, 1e4)  # Limit magnitude
        ...
        x_out_final = x_out + x
        x_out_final = torch.clamp(x_out_final, -10, 10)  # Output stabilization
