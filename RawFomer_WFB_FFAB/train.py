...
if __name__ == '__main__':
    ...
    use_pretrain = False
    pretrain_weights = os.path.join(save_weights_file, 'RawFormer_B_SID.pth')
	
    base_dir = os.path.join(os.path.dirname(__file__), 'Sony')

    if opt['dataset'] == 'SID':					
        train_input_paths = glob.glob(os.path.join(base_dir, 'short/0*_00_0.1s.ARW')) + glob.glob(os.path.join(base_dir, 'short/2*_00_0.1s.ARW'))
        train_gt_paths = []
        for x in train_input_paths:
            train_gt_paths += glob.glob(os.path.join(base_dir, 'long/*' + x[-17:-12] + '*.ARW'))

        test_input_paths = glob.glob(os.path.join(base_dir, 'short/1*_00_0.1s.ARW'))
        test_gt_paths = []
        for x in test_input_paths:
            test_gt_paths += glob.glob(os.path.join(base_dir, 'long/*' + x[-17:-12] + '*.ARW'))
        ...
        print("Train input path:", (train_input_paths))
        print("Train GT path:", (train_gt_paths))
        print("Train input files found:", len(train_input_paths))
        print("Loaded training samples:", len(train_data))
    ...
    for epoch in range(start_epoch, end_epoch + 1):
        ...
        for i, img in enumerate(tqdm.tqdm(dataloader_train)):
            ...
            # Input validation
            print(f"\nBatch {i}: Input stats - mean={input_raw.mean():.4f}, min={input_raw.min():.4f}, max={input_raw.max():.4f}")
            print(f"GT stats - mean={gt_rgb.mean():.4f}, min={gt_rgb.min():.4f}, max={gt_rgb.max():.4f}")
    
            if torch.isnan(input_raw).any() or torch.isinf(input_raw).any():
                print("!!! NaN/Inf in input batch !!!")
                continue			
            ...
            with torch.cuda.amp.autocast():
                ...
                print(f"Pred stats - mean={pred_rgb.mean():.4f}, min={pred_rgb.min():.4f}, max={pred_rgb.max():.4f}")

                loss = loss_criterion(pred_rgb, gt_rgb)
                if torch.isnan(loss):
                    print("!!! NaN LOSS DETECTED !!!")
                    print("Debugging info:")
                    print("Input range:", input_raw.min(), input_raw.max())
                    print("Pred range:", pred_rgb.min(), pred_rgb.max())
                    print("GT range:", gt_rgb.min(), gt_rgb.max())
